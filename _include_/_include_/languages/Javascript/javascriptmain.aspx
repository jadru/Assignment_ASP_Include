<%@ Page Title="" Language="C#" MasterPageFile="~/Master.master" AutoEventWireup="true" CodeFile="javascriptmain.aspx.cs" Inherits="languages_php_phpmain" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    <style type="text/css">
    .auto-style1 {
        width: 50%;
    }
    .auto-style2 {
        width: 32px;
        height: 201px;
    }
    .auto-style3 {

        color: #0066CC;
    }
    .auto-style6 {
        width: 990px;
        margin-left: 120px;
            height: 345px;
        }
        .auto-style24 {
            width: 32px;
            height: 234px;
        }
        .auto-style33 {
            width: 97px;
        }
        .auto-style39 {
            width: 300px;
        }
        .auto-style40 {
            height: 89px;
            padding-left: 20px;
        }
        .auto-style41 {
            margin-left: 40px;
        width: 694px;
    }
        .auto-style42 {
            margin-left: 160px;
        }
        .auto-style43 {
            height: 89px;
            padding-left: 20px;
            width: 135px;
        }
        .auto-style44 {
            width: 61%;
        }
        .auto-style45 {
            margin-left: 40px;
            width: 61%;
        }
    </style>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <table class="auto-style44">
    <tr>
        <td class="auto-style39" rowspan="4">
            <asp:Image ID="Image22" runat="server" Height="312px" ImageUrl="~/objects/images/home_language_btn/javascript.png" Width="302px" />
        </td>
        <td>
            <asp:Image ID="Image3" runat="server" Height="68px" ImageUrl="~/objects/images/php_study.png" Width="162px" />
        </td>
    </tr>
    <tr>
        <td><h1>JavaScript I</h1></td>
    </tr>
    <tr>
        <td class="auto-style3"><h3>JavaScript설치환경과 기본적인 개념 학습</h3></td>
    </tr>
    <tr>
        <td>
                <asp:ImageButton ID="ImageButton1" runat="server" Height="101px" ImageUrl="~/objects/images/php_startlesson_1.png" Width="337px" OnClick="ImageButton1_Click" />
        </td>
    </tr>
</table>
<p>
    &nbsp;</p>
    <table class="auto-style6">
        <tr>
            <td class="auto-style33">
    <asp:Image ID="Image6" runat="server" CssClass="auto-style2" ImageUrl="~/objects/images/connect_image.png" />
            </td>
            <td class="auto-style43">
                <asp:Image ID="Image20" runat="server" Height="13px" ImageUrl="~/objects/images/connect_image_width.png" Width="400px" />
            </td>
            <td>
                <table class="auto-style1">
    <tr>
        <td rowspan="4" class="auto-style39">
            <asp:Image ID="Image1" runat="server" Height="312px" ImageUrl="~/objects/images/home_language_btn/javascript.png" Width="302px" />
        </td>
        <td>
                <asp:Image ID="Image9" runat="server" Height="74px" ImageUrl="~/objects/images/php_quiz.png" Width="177px" />
        </td>
    </tr>
    <tr>
        <td><h1>JavaScript I</h1></td>
    </tr>
    <tr>
        <td class="auto-style3">JavaScript 최종 정리</td>
    </tr>
    <tr>
        <td>
                <asp:ImageButton ID="ImageButton2" runat="server" Height="101px" ImageUrl="~/objects/images/php_startlesson_1.png" Width="337px" />
        </td>
    </tr>
</table>
            </td>
        </tr>
        </table>
<p>
    &nbsp;&nbsp;</p>
<table class="auto-style44">
    <tr>
        <td class="auto-style39" rowspan="4">
            <asp:Image ID="Image4" runat="server" Height="312px" ImageUrl="~/objects/images/home_language_btn/javascript.png" Width="302px" />
        </td>
        <td>
            <asp:Image ID="Image5" runat="server" Height="68px" ImageUrl="~/objects/images/php_study.png" Width="162px" />
        </td>
    </tr>
    <tr>
        <td><h1>JavaScript II</h1></td>
    </tr>
    <tr>
        <td class="auto-style3"><h3>JavaScript의 문법 배우기 - 변수, 연산자, 제어구조</h3></td>
    </tr>
    <tr>
        <td>
                <asp:ImageButton ID="ImageButton3" runat="server" Height="101px" ImageUrl="~/objects/images/php_startlesson_1.png" Width="337px" />
        </td>
    </tr>
</table>
<p>
    &nbsp;</p>
<p>
    &nbsp;</p>
<p class="auto-style42">
    <asp:Image ID="Image13" runat="server" CssClass="auto-style24" ImageUrl="~/objects/images/connect_image.png" />
    </p>
<p>
    &nbsp;</p>
<p>
    &nbsp;</p>
<p>
    &nbsp;</p>
<table class="auto-style45">
    <tr>
        <td class="auto-style39" rowspan="4">
            <asp:Image ID="Image7" runat="server" Height="312px" ImageUrl="~/objects/images/home_language_btn/javascript.png" Width="302px" />
        </td>
        <td>
            <asp:Image ID="Image8" runat="server" Height="68px" ImageUrl="~/objects/images/php_study.png" Width="162px" />
        </td>
    </tr>
    <tr>
        <td><h1>JavaScript III</h1></td>
    </tr>
    <tr>
        <td class="auto-style3"><h3>JavaScript 만들기</h3></td>
    </tr>
    <tr>
        <td>
                <asp:ImageButton ID="ImageButton4" runat="server" Height="101px" ImageUrl="~/objects/images/php_startlesson_1.png" Width="337px" />
        </td>
    </tr>
</table>
<p>
</p>
<p>
</p>
<p class="auto-style42">
    <asp:Image ID="Image12" runat="server" CssClass="auto-style24" ImageUrl="~/objects/images/connect_image.png" />
</p>
<p>
    &nbsp;</p>
<p>
</p>
<p>
    &nbsp;</p>
<table class="auto-style44">
    <tr>
        <td class="auto-style39" rowspan="5">
            <asp:Image ID="Image10" runat="server" Height="312px" ImageUrl="~/objects/images/home_language_btn/javascript.png" Width="302px" />
        </td>
        <td">
            <asp:Image ID="Image11" runat="server" Height="68px" ImageUrl="~/objects/images/php_study.png" Width="162px" />
        </td>
    </tr>
    <tr>
        <td><h1>
            <asp:Image ID="Image21" runat="server" Height="68px" ImageUrl="~/objects/images/php_study.png" Width="162px" />
            </h1>
        </td>
    </tr>
    <tr>
        <td><h1>JavaScript IV</h1></td>
    </tr>
    <tr>
        <td><h3 class="auto-style3">JavaScript 디버깅</h3></td>
    </tr>
    <tr>
        <td>
                <asp:ImageButton ID="ImageButton5" runat="server" Height="101px" ImageUrl="~/objects/images/php_startlesson_1.png" Width="337px" />
        </td>
    </tr>
</table>
<p>
</p>
    <p>
</p>
<p>
</p>
<p class="auto-style42">
    <asp:Image ID="Image19" runat="server" CssClass="auto-style24" ImageUrl="~/objects/images/connect_image.png" />
</p>
<p>
</p>
<p>
</p>
<table class="auto-style41">
    <tr>
        <td rowspan="4" class="auto-style39">
            <asp:Image ID="Image17" runat="server" Height="312px" ImageUrl="~/objects/images/home_language_btn/javascript.png" Width="302px" />
        </td>
        <td class="auto-style40">
                <asp:Image ID="Image18" runat="server" Height="74px" ImageUrl="~/objects/images/php_quiz.png" Width="177px" />
        </td>
    </tr>
    <tr>
        <td><h1>JavaScript V</h1>
    </tr>
    <tr>
        <td class="auto-style3"><h3>JavaScript 최종 정리</h3></td>
    </tr>
    <tr>
        <td>
                <asp:ImageButton ID="ImageButton6" runat="server" Height="101px" ImageUrl="~/objects/images/php_startlesson_1.png" Width="337px" />
        </td>
    </tr>
</table>
</asp:Content>

